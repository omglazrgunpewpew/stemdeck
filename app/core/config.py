...
# Which separation backend the pipeline dispatches to. Only "demucs" is
# supported today; the value is validated when a job starts separating so a
# typo doesn't prevent the server from booting.
SEPARATOR_BACKEND = (
    os.environ.get("STEMDECK_SEPARATOR_BACKEND", "demucs").strip().lower() or "demucs"
)
